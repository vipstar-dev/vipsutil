...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := vipsutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(vipsutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := vipsutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := vipsutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := vipsutil.NewBlockFromBytes(blockBytes)
	if err != nil {
