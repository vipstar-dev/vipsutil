...
func (cs *CoinSet) TotalValue() (value vipsutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue vipsutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue vipsutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount vipsutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue vipsutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue vipsutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue vipsutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue vipsutil.Amount, coins []Coin) (Coins, error) {
...
// vipsutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() vipsutil.Amount {
	return vipsutil.Amount(c.txOut().Value)
}
